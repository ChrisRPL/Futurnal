/**
 * Model Selector Component for LLM Answer Generation
 *
 * Step 02: LLM Answer Generation
 * Allows users to select which model to use for answer synthesis.
 * Shows installation status and allows downloading models.
 *
 * Research Foundation:
 * - CausalRAG (ACL 2025): Different models for different use cases
...
export function ModelSelector() {
  const { selectedModel, setSelectedModel } = useSearchStore();
  ...
  const isInstalled = useCallback(
    (modelId: string) => {
      ...
      // Check if any installed model starts with this ID (for tag variations)
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          title={
            currentModelInstalled
              ? `Model: ${currentModel.label} (${currentModel.vram})`
        ...
        >
          ...
          <span className="text-white/40">({currentModel.hint})</span>
