Summary: Design central normalization service orchestrating format detection, parsing, chunking, and enrichment.

# 02 · Normalization Service Architecture

## Purpose
Design and implement the central `NormalizationService` that orchestrates the document normalization pipeline. This service acts as the single entry point for all connectors, coordinating format detection, adapter selection, Unstructured.io processing, chunking, metadata enrichment, and delivery to downstream sinks. The architecture must be extensible, testable, and maintainable while supporting diverse document formats.
...
- Idempotency guarantees via content hashing
...
- **Architecture Principle**: "Modular design with pluggable adapters" (modOpt pattern)
- **Privacy-First**: No content in logs, metadata-only audit events
- **Offline Operation**: All processing on-device, no network dependencies
...
from .adapter_registry import FormatAdapterRegistry
from .chunking_engine import ChunkingEngine, ChunkingConfig
from .metadata_enrichment import MetadataEnrichmentPipeline
...
class NormalizationSink(Protocol):
    """Protocol for downstream sinks receiving normalized documents."""

    def handle(self, element: dict) -> None:
        """Process normalized document element."""
        ...
...
class NormalizationService:
    ...
    async def normalize_document(
        ...
        *,
        ...
        try:
            ...
            normalized_doc.normalized_at = datetime.utcnow()
    ...
    def _log_normalization_success(
        ...
        self.audit_logger.record(AuditEvent(
            ...
            metadata={
                ...
                "total_chunks": len(document.chunks) if document.is_chunked else None,
    ...
    def get_metrics(self) -> dict:
        ...
        return {
            ...
            "success_rate": (
                self.documents_processed / (self.documents_processed + self.documents_failed)
...
- ✅ Central service coordinates entire normalization pipeline
- ✅ Format detection based on extension and metadata
- ✅ Adapter selection and routing working correctly
- ✅ Chunking applied conditionally based on configuration
- ✅ Metadata enrichment integrated
- ✅ Error handling routes failures to quarantine
- ✅ Idempotency via content hashing
- ✅ Privacy-aware logging (no content exposure)
- ✅ Metrics tracking for telemetry
- ✅ Async/await support for I/O operations
- ✅ Configurable via NormalizationConfig
- ✅ Integration with existing audit logging
- ✅ Sink delivery for downstream processing
...
### Integration Tests
- End-to-end normalization for each format
- Chunking enabled vs disabled
...
- File reading
- Format adapter processing
- Unstructured.io calls
- Sink delivery
- Audit logging
...
- **Transient**: Network errors, temporary resource unavailability → Retry
- **Format-specific**: Malformed content, unsupported features → Quarantine
- **System**: Memory exhaustion, disk full → Fail fast
- **Privacy**: Access denied, encryption → Log and skip
...
- Should format detection use file magic numbers in addition to extensions?
- How to handle documents with multiple formats (e.g., embedded files)?
- Should we support pipeline customization per source type?
- What's the retry strategy for transient failures?
- How to handle partial normalization success?
- Should we support batch processing of multiple documents?
- How to version the normalization pipeline itself?

## Dependencies

- FormatAdapterRegistry (Task 03)
- ChunkingEngine (Task 04)
- MetadataEnrichmentPipeline (Task 05)
- UnstructuredBridge (Task 07)
